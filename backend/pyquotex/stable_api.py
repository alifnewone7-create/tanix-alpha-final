...
class Quotex:
    ...
    async def connect(self):
        # close the PREVIOUS api first — it used to be replaced before close()
        # ran, which leaked the old websocket thread on every reconnect
        old_api = self.api
        if old_api is not None:
            try:
                await old_api.close()
            except Exception as e:
                logger.warning("Failed to close previous websocket: %s", e)
