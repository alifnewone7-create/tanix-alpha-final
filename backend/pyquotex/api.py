...
# hard upper bounds so a stalled socket can never freeze the whole event loop
WS_HANDSHAKE_TIMEOUT = 45
WS_JOIN_TIMEOUT = 10
...
class QuotexAPI:
    ...
    async def start_websocket(self):
        ...
        deadline = time.time() + WS_HANDSHAKE_TIMEOUT
        while True:
            ...
            if time.time() > deadline:
                logger.error(
                    ...
                    f"{WS_HANDSHAKE_TIMEOUT}s — giving up instead of waiting forever.")
                return False, "Websocket handshake timeout."
    ...
    async def close(self):
        if self.websocket_client:
            ...
            if self.websocket_thread:
                # bounded join: an unresponsive reader thread must never block
                # the event loop forever (the thread is a daemon anyway)
                self.websocket_thread.join(WS_JOIN_TIMEOUT)
                if self.websocket_thread.is_alive():
                    logger.warning(
                        "Websocket thread still alive after "
                        f"{WS_JOIN_TIMEOUT}s — abandoning it.")
            self.websocket_client = None
        return True
