...
# (connect, read) ceiling for every blocking HTTP call made during login
DEFAULT_HTTP_TIMEOUT = (15, 45)
...
class Browser(Session):
    ...
    def send_request(self, method, url, headers=None, **kwargs):
        ...
        # `requests` blocks the calling thread — and this is called straight from
        # the bot's event loop, so without a timeout a half-open connection to
        # Quotex freezes the WHOLE bot (asyncio.wait_for cannot cancel a
        # synchronous call). (connect, read) seconds.
